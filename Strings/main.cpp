...
void testStage6(void){
	...
	int k;
	char* s;
	const char* q = "Kevin Yee";
	const char *newword = "Hi my name is Kevin";
	char* newstring;

	printf("Running Stage 6\n");

	for (k = 0; k < 20; k += 1) {
			dst[k] = newword[k];
}
	newstring = utstrdup(dst);



s = utstrcpy(newstring, q);
printf(s);
printf("\n");
}
int main(void) {
