...
void readInput(const char* file_name) {
	...
	do {
		...
		if (StringIsEqualTo(&next_command, &inventory)) {
			...
			processSummarize();
		} else if (StringIsEqualTo(&next_command, &sku)) { // EE312 honors requirement
			processSKU();
...
int main(void){
	...
	sku = StringCreate("SKU"); // honors requirement
	...
	StringDestroy(&sku); // honors requirement
	StringDestroy(&quit);
